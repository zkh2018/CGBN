...
struct DevFp{
  ...
  inline __device__ DevFp as_bigint(env_t& bn_env, uint32_t *res, uint32_t* buffer, const env_t::cgbn_t& modulus, const uint64_t inv){
    ...
    one.set_zero(bn_env);
    one.set_one(bn_env);
    return y.mul(bn_env, one, res, buffer, modulus, inv);
  }
...
__global__ void kernel_alt_bn128_g1_add(cgbn_error_report_t* report, alt_bn128_g1 a, alt_bn128_g1 b, alt_bn128_g1 c, const uint32_t count, uint32_t *tmp_res, cgbn_mem_t<BITS>* max_value, cgbn_mem_t<BITS>* modulus, const uint64_t inv){
...
__global__ void kernel_alt_bn128_g1_reduce_sum_one_range(
  ...
  for(int i = first + instance; i < first + reduce_depth; i+= gridDim.x * local_instances){
    ...
    if(scalar.isequal(bn_env, dev_field_zero)){
      ...
      one.set_zero(bn_env);
      one.set_one(bn_env);
      DevFp a = scalar.mul(bn_env, one, res, buffer, local_modulus, inv);
...
int alt_bn128_g1_add(alt_bn128_g1 a, alt_bn128_g1 b, alt_bn128_g1 c, const uint32_t count, uint32_t *tmp_res, cgbn_mem_t<BITS>* max_value, cgbn_mem_t<BITS>* modulus, const uint64_t inv){
  ...
  kernel_alt_bn128_g1_add<<<blocks, threads>>>(report, a, b, c, count, tmp_res, max_value, modulus, inv);
...
int alt_bn128_g1_reduce_sum(
  ...
  kernel_alt_bn128_g1_reduce_sum<<<blocks, threads>>>(report, values, scalars, index_it, partial, counters, ranges_size, firsts, seconds, tmp_res, max_value, t_zero, t_one, field_zero, field_one, density, bn_exponents, modulus, inv);
...
int alt_bn128_g1_reduce_sum_one_range(
  ...
  kernel_alt_bn128_g1_reduce_sum_one_range<<<blocks, threads>>>(report, values, scalars, index_it, partial, counters, ranges_size, firsts, seconds, tmp_res, max_value, t_zero, field_zero, field_one, density, bn_exponents, modulus, inv);
  //CUDA_CHECK(cudaDeviceSynchronize());
  //CGBN_CHECK(report);
  return 0;
}
void alt_bn128_g1_reduce_sum(
  ...
  int step = 64;
  kernel_alt_bn128_g1_reduce_sum<<<blocks, threads>>>(report, partial_in, counters_in, partial_out, counters_out, ranges_size, tmp_res, max_value, t_zero, depth, step, modulus, inv);
  CUDA_CHECK(cudaDeviceSynchronize());
  CGBN_CHECK(report);
}
